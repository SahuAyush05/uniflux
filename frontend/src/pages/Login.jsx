  ...
  const handleSignup = (event) => {
    ...
    createUserWithEmailAndPassword(auth, email, password)
      .then((res) => {
        ...
        navigate(
          `./detail?name=${encodeURIComponent(name)}&ph=${encodeURIComponent(
            profilePictureUrl
          )}`,
          { replace: true }
        );
    ...
    try {
      ...
      navigate(
        `./detail?name=${encodeURIComponent(
          user.displayName
        )}&ph=${encodeURIComponent(user.photoURL)}`,
        { replace: true }
      );
    } catch (error) {
      console.error("Google Sign-In Error", error);
    ...
    >
      ...
      >
        <Box
          sx={{
            display: "flex",
            width: "100%", // Double the width to accommodate both forms
          }}
        >
          {/* Login Form */}
          {isLogin ? (
